...
namespace NexusStack.EFCore.Repository
{
    public partial class ServiceBase<TEntity, TKey> : AutoMapperRepository<TEntity, TKey>, IServiceBase<TEntity, TKey> where TEntity : class, IEntity<TKey>
    {
        ...
        {
            //物理删除无视软删除拦截器 IsDeleted = true 的数据照样可以删除
            //return this.Entities.Where(condition).DeleteFromQueryAsync(cancellationToken);
            return this.GetQueryableIgnoreQueryFilters().Where(condition).DeleteFromQueryAsync(cancellationToken);
        }
    ...
    public partial class ServiceBase<TEntity> : ServiceBase<TEntity, long>, IServiceBase<TEntity> where TEntity : class, IEntity<long>
    {
            ...
            params NpgsqlParameter[] parameters) where T : class, new()
        {
            ...
            await GetDbContext.Database.OpenConnectionAsync();

            using (var command = GetDbContext.Database.GetDbConnection().CreateCommand())
            {
                ...
                using (var reader = await command.ExecuteReaderAsync())
                {
                    ...
                    {
                        pagedResult.TotalItemCount = reader.GetInt32(0);
                    }
                    // Move to the second result set (paged data)
                    var dataList = new List<T>();
                    if (await reader.NextResultAsync())
                    {
                        while (await reader.ReadAsync())
                        {
                            // Map each row to the T entity
                            var entity = new T();
                    ...
                    pagedResult.Data = dataList;
                }
                await GetDbContext.Database.CloseConnectionAsync();
            }
