...
namespace NexusStack.EFCore.Repository.Query
{
    /// <summary>
    ...
    public abstract class QueryRepository<TEntity, TKey> : IQueryRepository<TEntity, TKey> where TEntity : class
    {
        ...
        public QueryRepository(MainContext dbContext)
            : this(dbContext, SpecificationEvaluator.Default)
        {
            DbContext = dbContext;
        }

        public QueryRepository(MainContext dbContext, ISpecificationEvaluator specificationEvaluator)
        {
            DbContext = dbContext;
        ...
        public IQueryable<TEntity> GetIgnoreQueryFiltersExpandable()
        {
            return GetQueryableIgnoreQueryFilters().AsExpandable();
        }

        public IQueryable<TEntity> GetQueryableIgnoreQueryFilters()
        {
        ...
        public virtual Task<bool> ExistsWithSoftDeleteAsync(Expression<Func<TEntity, bool>> condition, CancellationToken cancellationToken = default)
        {
            ArgumentNullException.ThrowIfNull(condition);
            return GetQueryableIgnoreQueryFilters().Where(condition).AnyAsync(cancellationToken);
        }
