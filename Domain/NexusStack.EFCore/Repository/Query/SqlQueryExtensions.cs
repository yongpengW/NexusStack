...
namespace NexusStack.EFCore.Repository.Query
{
    ...
    internal static class SqlQueryExtensions
    {
        ...
        private static async Task<List<T>> GetFromQueryAsync2<T>(this MainContext dbContext, string sql, IEnumerable<object> parameters, CancellationToken cancellationToken = default)
        {
        ...
        private static async Task<RelationalDataReader> ExecuteSqlQueryAsync(this MainContext dbContext, string sql, IEnumerable<object> parameters, CancellationToken cancellationToken = default)
        {
            if (dbContext == null)
            {
                throw new ArgumentNullException(nameof(dbContext));
